...

{
  hyprlandKeybindsConvert = ''
    ...
    # Assign apps // HyDE will try to use the default apps if values below are not set
    # $TERMINAL = kitty
    # $EDITOR = code
    # $EXPLORER = dolphin
    # $BROWSER = firefox
    ...
    bind = Alt_R, Control_R , exec, killall .waybar-wrapped || (env reload_flag=1 $scrPath/wbarconfgen.sh) # toggle waybar and reload config
    #bind = Alt_R, Control_R, exec, killall .waybar-wrapped || waybar # toggle waybar without reloading, this is faster
    ...
    bind = $mainMod, colon, exec, pkill -x rofi || $scrPath/keybinds_hint.sh c # launch keybinds hint
    bind = $mainMod, semicolon,exec, pkill -x rofi || $scrPath/emoji-picker.sh # launch emoji picker
