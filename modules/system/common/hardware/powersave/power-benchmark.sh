...
get_power_usage() {
    ...
    # Find all battery interfaces
    local battery_paths=()
    for bat_path in /sys/class/power_supply/BAT*; do
        if [[ -d "$bat_path" ]]; then
            battery_paths+=("$bat_path")
        fi
    done
    
    # Method 1: Check battery power draw (microwatts)
    ...
    # Method 2: Calculate from current and voltage
    ...
    # Method 3: Try AC power measurement
    if [[ $power_uw -eq 0 && -f /sys/class/power_supply/AC/power_now ]]; then
        power_uw=$(cat /sys/class/power_supply/AC/power_now 2>/dev/null || echo 0)
    fi
    ...
    # Convert from microwatts to watts and ensure dot decimal separator
    if command -v bc &> /dev/null; then
        echo "scale=2; $power_uw / 1000000" | bc -l | tr ',' '.'
    else
        # Fallback to awk if bc is not available
        awk "BEGIN {printf \"%.2f\", $power_uw / 1000000}" | tr ',' '.'
    fi
}
...
get_temperature() {
    ...
    if [[ -f /sys/class/thermal/thermal_zone0/temp ]]; then
        ...
        echo "scale=1; $temp / 1000" | bc -l | tr ',' '.'  # Convert to Celsius and ensure dot
    else
        echo "0"
    fi
}
